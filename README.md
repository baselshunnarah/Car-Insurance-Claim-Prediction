...
- **Libraries:** pandas, numpy, scikit-learn, TensorFlow/Keras, Keras Tuner, imbalanced-learn (SMOTE), matplotlib, seaborn
...
│
├── Car_Insurance_Data_P1.ipynb   # Main notebook (EDA + Modeling + Evaluation)
├── README.md                     # This file
└── data/
    └── car_insurance.csv         # Dataset (if included)
```
...
2. Open `Car_Insurance_Data_P1.ipynb` in Google Colab or Jupyter
3. Mount your Google Drive (if using Colab) and update the data path
...
- Threshold tuning (lower decision threshold to further boost recall on Class 1)
- XGBoost / LightGBM experimentation
- Cost-sensitive learning using `class_weight` parameter
